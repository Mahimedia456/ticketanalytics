...
import { fetchTicketDashboard } from "../services/ticketApi";
...
export default function TicketDashboardPage() {
  ...
  async function loadTicketDashboard() {
    ...
    try {
      const data = await fetchTicketDashboard(reportMonth);
      setRows(data.rows || []);
      setUpload(data.upload || null);
    } catch (err) {
      setError(err.message || "Unable to load ticket dashboard.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <DashboardActions
          onExportPDF={() =>
            exportDashboardPDF("ticket-dashboard-export", "Ticket Dashboard")
          }
          onExportExcel={() =>
      ...
      <div id="ticket-dashboard-export" className="space-y-6 pb-10">
        <DashboardHero
          title="Ticket Dashboard"
        ...
        {error ? (
          <div className="flex items-start gap-3 rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-red-200">
            ...
            <div>
              <p className="font-black">Unable to load ticket dashboard</p>
